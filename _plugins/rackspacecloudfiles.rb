		...
		def upload_file(fn)
			...
			if !@files.has_key?(fn)
				...
				open(fn, "r") do |io|
					counter = 0
					while (!io.eof)
						sha1.update(io.readpartial(65536))
					end
				end
				# And use it as the filename, but take the extension from the source file
				ext_name = File.extname(fn)
				obj_name = @upload_prefix + sha1.hexdigest + ext_name

				# Get the mime type from the extension
				mime_type = String(MIME::Types.type_for(ext_name)[0])
				...
				if !@files.has_key?(obj_name)
					...
					if @force_upload or !@cfc.object_exists?(obj_name)
						# Upload the file
						puts 'RackspaceCloudFiles: Uploading ' + obj_name + ' (' + mime_type + ')'
						@cfc.create_object(obj_name, false).write(open(fn, "r"), { 'Content-Type' => mime_type })
					end
